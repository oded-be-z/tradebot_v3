...
class Ultimate100TestSuite {
    ...
    async testQuery(query, expectedType, testName, category) {
        try {
            ...
            const testResult = {
                ...
                actualContent: response.data?.data?.content || 'NO_CONTENT'
            };
            ...
            switch (expectedType) {
                case 'refuse':
                    if (content.toLowerCase().includes('focus exclusively on financial') || 
                ...
                case 'financial':
                    ...
                    if (bullets === 4) {
                        ...
                        for (let i = 0; i < bulletLines.length; i++) {
                            ...
                            if (words > 12) { // Allow slight flexibility
                                testResult.issues.push(`Bullet ${i+1} has ${words} words (should be ≤10)`);
                            }
                        }
                    } else {
                ...
                case 'greeting':
                    ...
                    if (content.includes('Max here') && greetingBullets === 4) {
                        ...
                        testResult.issues.push('Greeting should follow 4-bullet format with Max intro');
                    }
                    break;

                case 'chart':
                    if (response.data.data.chartData || content.includes('chart') || content.includes('Chart')) {
                        testResult.success = true;
                    } else {
                ...
                case 'commodity':
                    ...
                    if (commodityBullets === 4 && !content.toLowerCase().includes('focus exclusively')) {
                        testResult.success = true;
                    } else {
                        testResult.issues.push('Commodity query should be accepted with 4-bullet format');
                    }
                    break;
            }
    ...
    recordResult(testResult, category, success) {
        ...
        if (success) {
            ...
            this.log(`FAIL: ${testResult.name} - ${testResult.issues?.join(', ') || testResult.error}`, 'fail');
        }
    }
    ...
    async runNonFinancialTests() {
        ...
        for (let i = 0; i < nonFinQueries.length; i++) {
            await this.testQuery(nonFinQueries[i], 'refuse', `NonFin-${i+1}: "${nonFinQueries[i]}"`, 'nonFinancial');
        }
    }
    ...
    async runStockCryptoTests() {
        ...
        for (let i = 0; i < stockCryptoQueries.length; i++) {
            await this.testQuery(stockCryptoQueries[i], 'financial', `Stock-${i+1}: "${stockCryptoQueries[i]}"`, 'stockCrypto');
        }
    }
    ...
    async runPortfolioTests() {
        ...
        for (let i = 0; i < portfolioQueries.length; i++) {
            await this.testQuery(portfolioQueries[i], 'financial', `Portfolio-${i+1}: "${portfolioQueries[i]}"`, 'portfolio');
        }
    }
    ...
    async runChartTests() {
        ...
        for (let i = 0; i < chartQueries.length; i++) {
            await this.testQuery(chartQueries[i], 'chart', `Chart-${i+1}: "${chartQueries[i]}"`, 'charts');
        }
    }
    ...
    async runCommodityTests() {
        ...
        for (let i = 0; i < commodityQueries.length; i++) {
            await this.testQuery(commodityQueries[i], 'commodity', `Commodity-${i+1}: "${commodityQueries[i]}"`, 'commodities');
        }
    }
...
<body>
    ...
    ${Object.keys(this.results.categories).map(cat => {
    ...
    <table>
        ...
        ${this.results.tests.map((test, i) => {
            const status = test.success ? '<span class="pass">PASS</span>' : '<span class="fail">FAIL</span>';
            return `<tr>
                <td>${i+1}</td>
                <td>${test.name}</td>
                <td>${test.query}</td>
    ...
    async run() {
        ...
        await this.log(`🎯 TESTING COMPLETE: ${this.results.passed}/${this.results.total} tests passed`);
