    ...
    async tryMultipleSources(symbol, displayPair) {
        ...
        if (process.env.POLYGON_API_KEY) {
            try {
                ...
                console.log(`[Forex] Polygon failed for ${symbol}: ${e.message}, trying alternatives...`);
            }
        }
        ...
        if (process.env.ALPHA_VANTAGE_API_KEY) {
            try {
                const alphaResult = await this.fetchFromAlphaVantage(symbol, displayPair);
                if (alphaResult.rate) return alphaResult;
            } catch (e) {
                console.log(`[Forex] Alpha Vantage failed for ${symbol}: ${e.message}, trying free sources...`);
            }
        }
