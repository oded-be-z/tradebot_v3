...
class CommoditiesService {
  constructor() {
    ...
    this.commodities = {
      ...
      OPEC: {
        symbol: "CL=F",
        name: "Crude Oil WTI (OPEC reference)",
        unit: "barrel",
        polygon: "C:USOIL",
      },
      BRENT: {
  ...
  async tryMultipleSources(symbol, commodity) {
    ...
    if (process.env.POLYGON_API_KEY && commodity.polygon) {
      try {
        ...
        console.log(
          `[Commodities] Polygon failed for ${symbol}: ${e.message}, trying alternatives...`,
        );
      }
    }
    ...
    try {
      ...
      console.log(
        `[Commodities] Yahoo failed for ${symbol}: ${e.message}, trying alternatives...`,
      );
    }
    ...
    if (process.env.ALPHA_VANTAGE_API_KEY) {
      try {
        ...
        console.log(
          `[Commodities] Alpha Vantage failed for ${symbol}: ${e.message}, trying free sources...`,
        );
      }
    }
  ...
  async fetchFromFreeAPIs(symbol, commodity) {
    ...
    if (
      ...
      try {
        const metalSymbol =
          symbol === "GOLD" || symbol === "GC"
            ? "XAU"
            : symbol === "SILVER" || symbol === "SI"
              ? "XAG"
              : symbol === "PLATINUM" || symbol === "PL"
                ? "XPT"
                : "XPD";
        ...
        console.log(
          `[Commodities] Metals API failed for ${symbol}: ${e.message}`,
        );
      }
    }
