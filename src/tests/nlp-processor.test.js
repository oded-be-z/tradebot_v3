...
describe("NLPProcessor", () => {
  ...
  describe("identifyQueryType", () => {
    test("should identify price queries", () => {
      expect(processor.identifyQueryType("What's AAPL price?")).toBe(
        "price_query",
      );
      expect(processor.identifyQueryType("How much does Tesla cost?")).toBe(
        "price_query",
      );
      expect(processor.identifyQueryType("What is Bitcoin worth?")).toBe(
        "price_query",
      );
    });

    test("should identify buy intent", () => {
      expect(processor.identifyQueryType("Should I buy Apple stock?")).toBe(
        "buy_intent",
      );
      expect(processor.identifyQueryType("Is it good to purchase MSFT?")).toBe(
        "buy_intent",
      );
    });
    ...
    test("should identify sell intent", () => {
      expect(
        processor.identifyQueryType("When should I sell my Tesla shares?"),
      ).toBe("sell_intent");
    });

    test("should identify analysis requests", () => {
      expect(processor.identifyQueryType("Can you analyze AAPL stock?")).toBe(
        "analysis_request",
      );
      expect(processor.identifyQueryType("Technical analysis of Bitcoin")).toBe(
        "analysis_request",
      );
    });

    test("should identify news requests", () => {
      expect(processor.identifyQueryType("Any news about Tesla?")).toBe(
        "news_request",
      );
      expect(processor.identifyQueryType("Latest updates on Apple")).toBe(
        "news_request",
      );
    });

    test("should identify comparisons", () => {
      expect(processor.identifyQueryType("Apple vs Microsoft stock")).toBe(
        "comparison",
      );
      expect(processor.identifyQueryType("Compare AAPL and MSFT")).toBe(
        "comparison",
      );
    });
  });
