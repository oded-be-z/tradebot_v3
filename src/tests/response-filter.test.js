...
describe("ResponseFilter", () => {
  ...
  describe("shouldBlockResponse", () => {
    ...
    test("should allow greetings", () => {
      const result = filter.shouldBlockResponse("Hello there", "greeting", 0.9);
  ...
  describe("generateRefusalMessage", () => {
    test("should generate appropriate refusal for non-financial queries", () => {
      ...
      expect(message.message).toContain(
        "not able to help with non-financial topics",
      );
  ...
  describe("filterResponse", () => {
    test("should not filter appropriate financial responses", () => {
      const response =
        "AAPL is currently trading at $150. The stock has shown strong performance this quarter.";
    ...
    test("should filter responses with non-financial content", () => {
      const response =
        "Here's a great recipe for apple pie that you should try!";
      const result = filter.filterResponse(
        response,
