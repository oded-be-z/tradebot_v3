class DisclaimerManager {
  constructor() {
    this.disclaimers = {
      general: {
        text: "Educational only. Consult advisor.",
        severity: "medium",
        placement: "end",
      },
  ...
  analyzeContent(content, queryType = "general") {
    ...
    if (
      ...
      this.isFinancialContent(lowerContent)
    ) {
  ...
  assessRiskLevel(content) {
    ...
    if (this.riskKeywords.medium_risk.some((term) => content.includes(term)))
      return "medium";
    return "low";
  }
  ...
  processResponse(
    ...
    if (session && session.disclaimerShown) {
      return {
        content: content,
        disclaimersAdded: [],
        skipped: true,
        reason: "Already shown in session",
      };
    }

    // Check if disclaimers are already present to avoid duplication
    if (this.hasExistingDisclaimers(content)) {
      return {
        content: content,
        disclaimersAdded: [],
        riskLevel: "unknown",
        contentType: "pre-processed",
        skipped: true,
        reason: "Disclaimers already present",
      };
    }
    ...
    // Mark disclaimer as shown immediately to prevent repeats
    if (session) {
  ...
  shouldAddDisclaimers(riskLevel, userPreferences = {}) {
    ...
    if (userPreferences.minimalDisclaimers === true && riskLevel === "low")
      return false;
  ...
  hasExistingDisclaimers(content) {
    if (!content || typeof content !== "string") return false;

    // Check for existing disclaimer patterns
