...
const EnhancedQueryAnalyzer =
  require("./server.js").EnhancedQueryAnalyzer ||
  ...
  for (const query of testQueries) {
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          console.log(
            `✅ Response received (${data.metadata?.queryType || "unknown"} query)`,
          );
          console.log(`   Topic extracted: ${data.metadata?.topic || "none"}`);
          console.log(
            `   Intent: ${data.metadata?.intentClassification?.classification || "unknown"}`,
          );
        } else {
...
function testSymbolMappingLogic() {
  ...
  testCases.forEach(({ query, expected }) => {
    ...
    console.log(
      `${result} "${query}" → ${found || "none"} (expected: ${expected})`,
    );
  });
}
...
if (require.main === module) {
  ...
  testNLPImprovements().catch((error) => {
    console.log(
      "⚠️  Server API test failed. Make sure the server is running with: npm start",
    );
