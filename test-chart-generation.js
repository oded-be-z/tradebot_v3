  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      if (response.success) {
        ...
        const chartType =
          response.chart?.type ||
          response.data?.chart?.type ||
          response.data?.chartData?.type;
        ...
        if (hasChart) {
          ...
          const imageUrl =
            response.chart?.imageUrl ||
            response.data?.chart?.imageUrl ||
            response.data?.chartData?.imageUrl;
          if (imageUrl && imageUrl.startsWith("data:image/png;base64,")) {
            console.log(
              `   🖼️  Base64 image detected (${imageUrl.length} chars)`,
            );
        ...
        console.log(
          `   📄 Response content length: ${(response.data?.content || "").length} chars`,
        );
      } else {
        console.log(
          `   ❌ FAIL - API error: ${response.error || "Unknown error"}`,
        );
      }
    } catch (error) {
