...
class ModernResponseFormatter {
    constructor() {
        this.modernGreetings = [
            "Hey! I'm Max. Drop a ticker - let's make money.",
            "Markets are moving. What are we trading?",
            "Ready to print? Give me a symbol."
        ];
    }
...
class ChartGenerator {
    ...
    generateChart(queryInfo, marketData = null) {
        ...
        switch (queryInfo.queryType) {
            case 'stock':
            case 'crypto':
                return this.generatePriceChart(queryInfo.topic);
            case 'comparison':
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        if (queryInfo.needsChart) {
            chartData = chartGenerator.generateChart(queryInfo);
        ...
        try {
            ...
            // Enhanced fallback response with modern formatting
            const fallbackContent = queryInfo.topic 
                ? `${queryInfo.topic} data is updating - markets are moving fast. Try again in a sec.`
                : "Market data feed interrupted. Drop me a ticker and I'll get you the latest.";
